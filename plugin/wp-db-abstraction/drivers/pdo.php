	...
	*/
	...
	function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
        ...
        if ($this->pdo_type != 'sqlsrv') {
		    ...
		    $dsn = $this->pdo_type . ':server=' . $dbhost;
			$connection_info['Database'] = $dbname;
		}
		...
		try {
			$this->dbh = new PDO($dsn, $dbuser, $dbpassword, $connection_info);
		} catch (PDOException $e) {
			// We're not really doing anything with this, but it MUST be caught
			// to show the error handler below properly
		}
		if ( !$this->dbh ) {
...
<p>If you don\'t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="http://wordpress.org/support/">WordPress Support Forums</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser ), 'db_select_fail' );
	...
	function db_connect( $query = "SELECT" ) {
        ...
        if ($this->pdo_type != 'sqlsrv') {
		    if (stristr($details[ 'db_host' ], ':')) {
				$dbhost_parts = explode(':', $details[ 'db_host' ]);
				$dsn[] = 'host=' . $dbhost_parts[0] . ';port=' . $dbhost_parts[1];
			} else {
				$dsn[] = 'host=' . $details[ 'db_host' ];
			}
			$dsn[] = 'dbname=' . $details[ 'db_name' ];
			$dsn = $this->pdo_type . ':' . implode(';', $dsn);
        } else {
		    $dsn = $this->pdo_type . ':' . $details[ 'db_host' ];
