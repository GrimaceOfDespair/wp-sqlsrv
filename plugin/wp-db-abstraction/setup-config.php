...
if (file_exists(ABSPATH . '../wp-config.php') && ! file_exists(ABSPATH . '../wp-settings.php'))
	wp_die("<p>The file 'wp-config.php' already exists one level above your WordPress installation. If you need to reset any of the configuration items in this file, please delete it first. You may try <a href='" . setup_get_url() . "install.php'>installing now</a>.</p>");
...
// Add check for available drivers
...
foreach($drivers as $extension) {
	if (extension_loaded($extension) && file_exists(ABSPATH . 'wp-content/mu-plugins/wp-db-abstraction/drivers/' . $extension . '.php')) {
		if ($extension === 'pdo') {
			$avail_drivers = PDO::getAvailableDrivers();
			foreach ($avail_drivers as $pdo_driver) {
				$pd = 'pdo_' . $pdo_driver;
				if (in_array($pdo_driver, $pdo_drivers)) {
...
<link rel="stylesheet" href="<?php echo setup_get_url()?>wp-admin/css/install.css" type="text/css" />

</head>
<body>
<h1 id="logo"><img alt="WordPress" src="<?php echo setup_get_url()?>wp-admin/images/wordpress-logo.png" /></h1>
<?php
...
<p>Welcome to WordPress with the Wordpress Database Abstraction Plugin. You may have been redirected here from the mysql specific config setup page.  Before getting started, we need some information on the database. You will need to know the following items before proceeding.</p>
<ol>
	<li>Database name</li>
...
<form method="post" action="?step=2">
	...
	<table class="form-table">
		...
		<tr>
			<th scope="row"><label for="dbtype">Database Type</label></th>
			<td>
				...
				foreach($valid_drivers as $option) {
					if ($option == 'mysql') {
						...
						echo "<option value=\"$option\">SQL Server using dblib/ntwdblib driver (MSSQL)</option>\n";
					} elseif ($option == 'pdo_mysql') {
						echo "<option value=\"$option\">PDO MySQL</option>\n";
					} elseif ($option == 'pdo_sqlsrv') {
						echo "<option value=\"$option\">PDO SqlSrv</option>\n";
					}
				}
				?>
				</select>
...
<?php
	...
	require_wp_db();
	if ( ! empty( $wpdb->error ) ) {
		define('WP_SITEURL', setup_get_url());
		$back = '<p class="step"><a href="?step=1" onclick="javascript:history.go(-1);return false;" class="button">Try Again</a></p>';
	...
	if ( ! $no_api ) {
		...
		function get_bloginfo() {
			return ( ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . str_replace( $_SERVER['PHP_SELF'], '/wp-content/mu-plugins/wp-db-abstraction/setup-config.php', '' ) );
		}
		/**#@-*/
	...
	foreach ($configFile as $line_num => $line) {
		switch (substr($line,0,16)) {
			...
			case '$table_prefix  =':
				...
				$log_file = WP_CONTENT_DIR . DIRECTORY_SEPARATOR . 'queries.log'; 
				$configFile[$line_num] .= "\n\n/** Query Logging Settings */\ndefine('SAVEQUERIES', FALSE);\ndefine('QUERY_LOG', '" . $log_file . "');\n";
				break;
