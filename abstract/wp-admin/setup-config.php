...
if (!is_dir(ABSPATH . 'wp-includes/wp-db/drivers/')) {
	wp_die('Your wordpress install is missing the wp-db/drivers directory needed to access the database');
}
...
foreach($drivers as $extension) {
	if (extension_loaded($extension) && file_exists(ABSPATH . 'wp-includes/wp-db/drivers/' . $extension . '.php')) {
		if ($extension === 'pdo') {
...
if ($validated_database == false)
	wp_die('Your PHP installation appears to be missing all the database extensions (' . implode(', ', $drivers) . ') which are supported by WordPress with Patch.');
