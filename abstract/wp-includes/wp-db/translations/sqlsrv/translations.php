...
class SQL_Translations
{
    ...
    function translate_general($query)
    {
        ...
        if (stristr($query, 'SELECT COUNT(DISTINCT(wp_users.ID))') !== FALSE) {
            $query = str_ireplace(
				'SELECT COUNT(DISTINCT(wp_users.ID))', 
				'SELECT COUNT(DISTINCT(wp_users.ID)) as Computed', $query);
        }
    ...
    {
        if ($query == "SELECT COUNT(NULLIF(meta_value LIKE '%administrator%', FALSE) as Computed), COUNT(NULLIF(meta_value LIKE '%editor%', FALSE)), COUNT(NULLIF(meta_value LIKE '%author%', FALSE)), COUNT(NULLIF(meta_value LIKE '%contributor%', FALSE)), COUNT(NULLIF(meta_value LIKE '%subscriber%', FALSE)), COUNT(*) FROM wp_usermeta WHERE meta_key LIKE 'wp_capabilities'") {
            $query = "SELECT 
    (SELECT COUNT(*) FROM wp_usermeta WHERE meta_key LIKE 'wp_capabilities' AND meta_value LIKE '%administrator%') as ca, 
    (SELECT COUNT(*) FROM wp_usermeta WHERE meta_key LIKE 'wp_capabilities' AND meta_value LIKE '%editor%') as cb, 
    (SELECT COUNT(*) FROM wp_usermeta WHERE meta_key LIKE 'wp_capabilities' AND meta_value LIKE '%author%') as cc, 
    (SELECT COUNT(*) FROM wp_usermeta WHERE meta_key LIKE 'wp_capabilities' AND meta_value LIKE '%contributor%') as cd, 
    (SELECT COUNT(*) FROM wp_usermeta WHERE meta_key LIKE 'wp_capabilities' AND meta_value LIKE '%subscriber%') as ce, 
    COUNT(*) as c FROM wp_usermeta WHERE meta_key LIKE 'wp_capabilities'";
		} 
		
        if (stristr($query, "SELECT DISTINCT TOP 50 (wp_users.ID) FROM wp_users") !== FALSE) {
            $query = str_ireplace(
				"SELECT DISTINCT TOP 50 (wp_users.ID) FROM", 
				"SELECT DISTINCT TOP 50 (wp_users.ID), user_login FROM", $query);
		}
   	    return $query;
    }
